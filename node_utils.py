...
def process_image_with_mask(image, mask,width, height):

    mask_tensor=mask.reshape((-1, 1, mask.shape[-2], mask.shape[-1])).movedim(1, -1).expand(-1, -1, -1, 3)

    input_image_resized = tensor2pil_upscale(image, width, height) 
    mask_pil=tensor2pil_upscale(mask_tensor, width, height) 
