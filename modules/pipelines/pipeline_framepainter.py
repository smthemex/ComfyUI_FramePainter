...
from ..sparse_control_encoder import SparseControlEncoder
from ..unet_spatio_temporal_condition_edit import UNetSpatioTemporalConditionEdit
...
@dataclass
class FramePainterPipelineOutput(BaseOutput):
    r"""
    Output class for sketch-based image editing pipeline.
...
class FramePainterPipeline(DiffusionPipeline):
    r"""
    Pipeline to edit images from an input image and a sketch image using FramePainter.
    ...
    Args:
        ...
        unet ([`UNetSpatioTemporalConditionEdit`]):
            A `UNetSpatioTemporalConditionEdit` to denoise the encoded image latents.
    ...
    def _encode_image(self, image_embs, device, num_videos_per_prompt, do_classifier_free_guidance):
        ...
        image_embeddings = image_embs.unsqueeze(1)
    ...
    def __call__(
        ...
        edit_condition:Optional[torch.FloatTensor] = None,
        height: int = 576,
        width: int = 1024,
        ...
        image_embeddings = self._encode_image(image_embs, device, num_videos_per_prompt, do_classifier_free_guidance)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    N = edit_output['output'].shape[0]
                    edit_output['scale'] = torch.tensor(edit_output['scale']).to(latent_model_input).repeat(N)[:, None, None, None]
                    edit_output['scale'][:N // 2] *= 0
                ...
                noise_pred = self.unet(
                    ...
                    conditional_controls=edit_output,
                    return_dict=False,
                )[0]
                ...
                if guidance_scale_decay == "none":
                    ...
                    cur_guidance_scale_points = \
                        (self.guidance_scale-1.0) * (1.0 - i/len(timesteps)) ** 2 + 1
                else:
        ...
        return FramePainterPipelineOutput(frames=frames)
